...
#             item.is_one_handed = True
    ...
    def _is_item_available(self, item):
        return not item.removed and not self._is_unavailable_in_version(item)

    def _is_or_item_available(self, item):
        return not item.removed and not self._is_unavailable_in_version(item)

    def _is_unavailable_in_version(self, item):
        blocked = UNAVAILABLE_ITEMS_BY_VERSION.get(self.game_version)
        return bool(blocked) and getattr(item, 'ankama_id', None) in blocked
