...
<p>Prospecting (PP) is the stat that decides your chance to drop an item or resource from a monster. Every character starts at <strong>100 prospecting</strong> (120 for Enutrofs on Dofus Retro), and that is the baseline every drop rate is quoted at. When the <a href="/encyclopedia/">encyclopedia</a> says an item drops at 1%, that is the chance at 100 PP. Prospecting also raises the Kamas monsters drop, so it pays for itself while you farm.</p>
...
<p>La prospection (PP) est la stat qui décide de ta chance de dropper un objet ou une ressource sur un monstre. Chaque personnage démarre à <strong>100 de prospection</strong> (120 pour les Enutrofs sur Dofus Retro), et c'est la base à laquelle tous les taux de drop sont donnés. Quand l'<a href="/encyclopedia/">encyclopédie</a> dit qu'un objet drop à 1%, c'est la chance à 100 PP. La prospection augmente aussi les Kamas lâchés par les monstres, donc elle se rentabilise pendant que tu farmes.</p>
...
<p>La prospección (PP) es la estadística que decide tu probabilidad de soltar un objeto o recurso de un monstruo. Cada personaje empieza con <strong>100 de prospección</strong> (120 para los Anutrofs en Dofus Retro), y esa es la base a la que se dan todas las tasas de drop. Cuando la <a href="/encyclopedia/">enciclopedia</a> dice que un objeto cae al 1%, esa es la probabilidad a 100 PP. La prospección también sube los Kamas que sueltan los monstruos, así que se rentabiliza mientras farmeas.</p>
...
<p>Prospektion (PP) ist der Wert, der deine Chance bestimmt, einen Gegenstand oder eine Ressource von einem Monster zu droppen. Jede Figur startet mit <strong>100 Prospektion</strong> (Enutrofs beginnen in Dofus Retro mit 120), und das ist die Basis, auf die sich jede Drop-Rate bezieht. Wenn die <a href="/encyclopedia/">Enzyklopädie</a> sagt, ein Gegenstand droppt zu 1%, ist das die Chance bei 100 PP. Prospektion erhöht auch die Kamas, die Monster fallen lassen, also zahlt sie sich beim Farmen von selbst aus.</p>
...
<p>Overmaging an item with an "exotic" AP, MP or range bonus (one the item never had) is powerful, but the game counts only <strong>one</strong> exo AP, one exo MP and one exo range across your whole set. You can put a +1 AP exo on three different items; only one of them counts. Plan around a single exo per stat, not a stack.</p>
...
<p>Sur-forger un objet avec un bonus "exotique" de PA, PM ou portée (un bonus que l'objet n'avait pas à l'origine), c'est puissant, mais le jeu ne compte qu'<strong>un seul</strong> PA exo, un seul PM exo et une seule portée exo sur tout ton équipement. Tu peux mettre un exo +1 PA sur trois objets différents ; un seul comptera. Prévois un exo unique par stat, pas une pile.</p>
...
<p>Sobreforjar un objeto con un bonus "exótico" de PA, PM o alcance (uno que el objeto no tenía de origen) es potente, pero el juego cuenta solo <strong>un</strong> PA exótico, un PM exótico y un alcance exótico en todo tu equipo. Puedes poner un exo de +1 PA en tres objetos distintos; solo contará uno. Planifica un único exo por estadística, no una pila.</p>
...
<p>Sobreforjar um item com um bônus "exótico" de PA, PM ou alcance (um que o item não tinha de origem) é forte, mas o jogo conta só <strong>um</strong> PA exótico, um PM exótico e um alcance exótico em todo o seu equipamento. Você pode colocar um exo de +1 PA em três itens diferentes; só um vai contar. Planeje um único exo por estatística, não uma pilha.</p>
...
<p>Einen Gegenstand mit einem "exotischen" AP-, MP- oder Reichweiten-Bonus zu übermagen (einen, den der Gegenstand nie hatte), ist stark, aber das Spiel zählt über deine gesamte Ausrüstung nur <strong>einen</strong> Exo-AP, einen Exo-MP und eine Exo-Reichweite. Du kannst drei verschiedenen Gegenständen einen Exo mit +1 AP geben; nur einer zählt. Plane mit einem einzigen Exo pro Wert, nicht mit einem Stapel.</p>
...
<p>The size of that question is worth a moment. A character has sixteen slots to fill. On Dofus 3 the catalogue offers well over a hundred candidates for every single one of them, and those sixteen slots can be filled in <strong>more than 10 to the power of 37</strong> different ways, a 1 followed by thirty-seven zeros. Touch and Dofus 2 are a little narrower, past 10 to the power of 36; Retro, whose catalogue is smaller, still passes 10 to the power of 28. Nothing could try those one at a time, and this doesn't try: it works out which set wins without visiting them, the way you can prove the shortest route across a city without walking every street.</p>
...
<p>La taille de la question mérite qu'on s'y arrête. Un personnage a seize emplacements à remplir. Sur Dofus 3, le catalogue propose bien plus d'une centaine de candidats pour chacun d'eux, et ces seize emplacements peuvent être remplis de <strong>plus de 10 puissance 37</strong> façons différentes, un 1 suivi de trente-sept zéros. Touch et Dofus 2 sont un peu plus étroits, au-delà de 10 puissance 36 ; Rétro, dont le catalogue est plus petit, dépasse encore 10 puissance 28. Rien ne pourrait les essayer une par une, et lui n'essaie pas : il détermine quel set gagne sans les visiter, comme on peut prouver le plus court chemin dans une ville sans parcourir chaque rue.</p>
...
<p>Vale la pena detenerse en el tamaño de la pregunta. Un personaje tiene dieciséis ranuras que llenar. En Dofus 3 el catálogo ofrece bastante más de cien candidatos para cada una, y esas dieciséis ranuras se pueden llenar de <strong>más de 10 elevado a 37</strong> maneras distintas, un 1 seguido de treinta y siete ceros. Touch y Dofus 2 son algo más estrechos, por encima de 10 elevado a 36; Retro, con un catálogo más pequeño, todavía supera 10 elevado a 28. Nada podría probarlas una a una, y esto no lo intenta: averigua qué conjunto gana sin visitarlas, igual que se puede demostrar el camino más corto por una ciudad sin recorrer cada calle.</p>
...
<p>Vale parar um instante no tamanho da pergunta. Um personagem tem dezesseis slots pra preencher. No Dofus 3 o catálogo oferece bem mais de cem candidatos pra cada um, e esses dezesseis slots podem ser preenchidos de <strong>mais de 10 elevado a 37</strong> maneiras diferentes, um 1 seguido de trinta e sete zeros. Touch e Dofus 2 são um pouco mais estreitos, acima de 10 elevado a 36; o Retro, com um catálogo menor, ainda passa de 10 elevado a 28. Nada conseguiria testar isso um por um, e ela nem tenta: descobre qual conjunto vence sem visitar todos, do mesmo jeito que dá pra provar o caminho mais curto por uma cidade sem andar por cada rua.</p>
...
<p>Die Größe dieser Frage ist einen Moment wert. Eine Figur hat sechzehn Plätze zu füllen. Auf Dofus 3 bietet der Katalog für jeden einzelnen davon weit über hundert Kandidaten, und diese sechzehn Plätze lassen sich auf <strong>mehr als 10 hoch 37</strong> verschiedene Arten füllen, eine 1 mit siebenunddreißig Nullen. Touch und Dofus 2 sind etwas enger, jenseits von 10 hoch 36; Retro, dessen Katalog kleiner ist, kommt immer noch über 10 hoch 28. Nichts könnte die einzeln durchprobieren, und das versucht es auch nicht: Es ermittelt, welches Set gewinnt, ohne sie zu besuchen, so wie man den kürzesten Weg durch eine Stadt beweisen kann, ohne jede Straße abzulaufen.</p>
...
<p>La Puissance booste tous tes dégâts élémentaires d'un coup et mérite presque toujours un gros poids. Les Dommages fixes (+dom) sont rois sur les sorts multi-coups à faible base ; les dégâts en % scalent avec les gros coups. Les stats de dégâts en % sont une nouveauté Dofus 2 : on les trouve sur Dofus 3, la bêta et Dofus 2, mais pas sur Touch ni Rétro, où tu comptes plutôt sur la Puissance et les dégâts fixes. Rétro nomme la Puissance comme son propre jeu, % de dommages, et c'est cette ligne que tu pondères là-bas. Le taux de critique est top <em>si</em> tes crits ajoutent vraiment un bonus qui compte : vérifie le sort avant de courir après, et sache que <a href="/guides/critical-hits/">le fonctionnement des coups critiques dépend de ta version</a>.</p>
...
<p>La Potencia sube todo tu daño elemental de golpe y casi siempre merece un peso alto. Los Daños fijos (+daño) brillan en hechizos multigolpe de base baja; el daño en % escala con los golpazos. Las estadísticas de daño en % son una novedad de Dofus 2: aparecen en Dofus 3, la beta y Dofus 2, pero no en Touch ni Retro, donde te apoyas en la Potencia y el daño fijo. Retro nombra la Potencia como su propio juego, % de daños, y esa es la línea que ponderas allí. La tasa de crítico es genial <em>si</em> tus críticos suman un bonus que de verdad cuenta: mira el hechizo antes de ir a por ella, y ten en cuenta que <a href="/guides/critical-hits/">cómo funcionan los golpes críticos depende de tu versión</a>.</p>
...
<h2>Die Multiplikatoren: Schlagkraft, Schaden, Kritisch</h2>
<p>Schlagkraft hebt deinen gesamten Elementarschaden auf einmal und verdient fast immer ein hohes Gewicht. Fixer Schaden (+Schaden) ist am stärksten bei Multi-Treffer-Zaubern mit niedriger Basis; %-Schaden skaliert mit großen Treffern. Die Prozent-Schadenswerte sind eine Dofus-2-Neuerung: sie tauchen auf Dofus 3, der Beta und Dofus 2 auf, aber nicht auf Touch oder Retro, wo du stattdessen auf Schlagkraft und festen Schaden setzt. Retro benennt die Schlagkraft wie sein eigenes Spiel, % Schaden, und das ist die Zeile, die du dort gewichtest. Kritische Trefferrate ist super, <em>wenn</em> deine Kritischen wirklich einen spürbaren Bonus draufpacken: schau den Zauber an, bevor du ihr hinterherjagst, und beachte, dass <a href="/guides/critical-hits/">wie kritische Treffer funktionieren, von deiner Version abhängt</a>.</p>
...
<p>Erstens die <strong>101-Grenze</strong>: das gesamte Over/Exo-Gewicht auf einem einzelnen Wert kann nie über 101 gehen, und der Simulator markiert jeden Wert, bei dem dein Ziel sie reißt. Zweitens gelingt eine Zeile, die am Ende <strong>30 Gewicht oder mehr über dem eigenen Wurf des Gegenstands</strong> liegt, nur bei einem kritischen Erfolg, üblicherweise um 1% pro Rune geschätzt: AP, BP, Reichweite, eine Beschwörung und % Zauberschaden ab dem zweiten Punkt. In Dofus Retro wiegt die Kritisch-Rune 30 statt 10, also ist ein über den Wurf des Gegenstands gedrückter Kritischer dort ebenfalls eine 1%-Zeile. Das Tool kennzeichnet diese Zeilen, damit du sie als langen Grind einplanst, nicht als schnelle Nummer.</p>
...
<p>In Dofus 3, der Beta und Dofus 2 steht der Bonus jedes Begleiters in den Spieldaten, die Fashionista liest ihn also direkt: Ein Maunzmiez zeigt seine Schlagkraft und sein Ausweichen, ein SchnuffWuff seine Weisheit, und der Optimierer behandelt sie wie jedes andere Teil. Der Haken liegt bei dir, nicht beim Werkzeug: Ein Begleiter liefert diese Zahlen nur, solange er versorgt wird. Der Build setzt einen gesunden Begleiter voraus.</p>
...
<p>Power, which Retro labels % Damage, flat +Damage and +Critical Damage are damage stats. They do not touch a heal, and neither does +Damage in the element of your heal. A healer wearing damage gear is wearing statistics that never fire, which is the most common way to build one badly.</p>
...
<p>La Puissance, que Rétro nomme % de dommages, les Dommages plats et les Dommages critiques sont des stats de dégâts. Ils ne touchent pas un soin, et les Dommages dans l'élément de ton soin non plus. Un soigneur en stuff de dégâts porte des statistiques qui ne se déclenchent jamais, et c'est la façon la plus courante de rater ce build.</p>
...
<p>Potencia, que Retro llama % de daños, Daños planos y Daños críticos son estadísticas de daño. No tocan una curación, ni tampoco los Daños en el elemento de tu curación. Un sanador con equipo de daño lleva estadísticas que nunca se disparan, y es la forma más común de arruinar este build.</p>
...
<p>Potência, que o Retro chama % de dano, Danos planos e Danos críticos são estatísticas de dano. Não tocam uma cura, e os Danos no elemento da sua cura também não. Um curandeiro em equipamento de dano usa estatísticas que nunca disparam, e essa é a maneira mais comum de errar este build.</p>
